	...
	if !hasPersistedIntent {
		...
		if strictArtifacts && len(acceptedArtifacts) == 0 {
			...
			if registration.Enabled {
				...
				intent.IcebergSchema, err = icebergreg.InferDurableIcebergSchema(ctx, srcConn.Engine, sourceDSN, opts.NormalizedSourceMode(), strings.TrimSpace(opts.Table), sourceQuery, opts.RecordPath)
				if err != nil {
