...
Trino, Cassandra, and SQLite now use their respective LogicalType mappers and shared conversion/storage path. Trino arrays retain structure while fallback elements resolve recursively; time-with-time-zone, ROW/MAP/IP/interval, and unknown plugin types are unknown fallback. Cassandra `varint`, unconstrained decimal, collections/UDTs, and inet are unknown fallback; its connector preserves `uint64` as decimal text, retains blobs as bytes, serializes UUIDs canonically, and uses lossless conversion instead of `fmt.Sprintf`. SQLite maps only explicit declarations and treats arbitrary/empty declarations and unconstrained numeric as unknown fallback; runtime values must still meet the declared semantic type.

## Mongo inference migration

Mongo now infers a LogicalType from all supplied sample documents rather than the first non-null value. Safe numeric promotion is explicit and rejects `int64` values outside exact float64 integer range; missing/null fields become nullable. Arrays infer elements recursively, while documents and unsupported BSON values use canonical Extended JSON fallback. ObjectID is unknown (not UUID), BSON Timestamp is unknown to preserve both `T` and `I`, and Decimal128 derives a common exact precision/scale when possible. Incompatible fields fall back losslessly instead of becoming zero or null. `InferMongoSchemaResult` provides structured warnings for fallback decisions; existing schema APIs remain compatibility wrappers.
